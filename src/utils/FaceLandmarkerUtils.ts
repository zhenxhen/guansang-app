...
export const drawBlendShapes = (el: HTMLElement, blendShapes: any[]) => {
  ...
  for (const blendshape of faceBlendshapes) {
    ...
    if (runningMode === "VIDEO") {
      ...
      const smoothingFactor = isMobile ? 0.3 : 0.5; // 더 빠른 반응을 위해 계수 낮춤
    ...
    if (container) {
      ...
      if (valueElement) {
        ...
        // 디버깅용 - 콘솔에 값과 클래스 출력 (개발 중에만 사용)
        if (categoryName === 'mouthSmile_L' || categoryName === 'eyeBlink_L') {
          console.log(`${categoryName}: ${percentage.toFixed(1)}% -> class: ${valueClass}`);
        }
      }
